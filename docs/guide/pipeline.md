...
plain subject the pipeline writes itself. The body has no length bound. Control
characters git or a terminal would choke on are stripped once from the
assembled message, covering both the scribe's answer and the stage's summary at
the history boundary. The pipeline produces the status line's outcome and
routing as single-line values; commit-message assembly asserts that invariant
instead of coercing their content. A scribe that dies or answers with nothing
degrades to the stage's own summary — the commit is never delayed for prose —
and says so on the event stream.
