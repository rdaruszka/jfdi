...
a local URL and watch the same read-only run status in a browser instead. Its
Settings panel can stage, reload, validate, and save `.jfdi/config.json`; no
change applies while you type. Cards
