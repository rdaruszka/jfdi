...
- **round** — one feedback cycle ending at another agent: Implementation → gate → reviews, with an in-stage gate-fix cycle (capped at 10) after either Implementation or QA makes the gate red (cap: `pipeline.maxRounds`). Gate failures alone do not consume a round; a post-QA fix that widens QA's initial path scope does.
