...
The **coordinator** watches `board.md` (Obsidian Kanban format), dispatches each ready card into its own **git worktree** on branch `jfdi/<ticket-id>`, and runs a per-ticket pipeline of fresh Claude Code or Codex sessions: **Implementation → mechanical gate → Code Review → QA**, with feedback rounds bounded by the reviewers' rejection budgets (`pipeline.maxRejections`, Code Review 2 and QA 1 by default). A gate failure feeds back into the session whose handoff made it red *within* the round for up to 3 fix sessions; a fourth red attempt blocks directly. Agents never commit: the pipeline commits each session's handoff itself, with a **scribe** session writing the message. One phase comment per stage and round folds together that message, the stage's decisions, status, and usage; gate-fix commit messages are included in their round's Implementation or QA comment. **Integration** is coordinator-owned and globally serialized — one merge at a time: merge the target branch into the ticket branch, rerun the gate, then land that tested tree on the target as a merge commit (target's prior head first parent, signed-off branch head second). Every transition appends to the project's `events.jsonl` under `~/.jfdi/projects/<project-key>/`; `state.json` is a derived snapshot; both live front ends render run status purely from that stream, while the web ticket detail reads the pipeline-written run logs read-only for full session output.
...
1. **Renderer separation.** All run status renders from `events.jsonl`/`state.json` only; renderers may also read pipeline-written run logs read-only for full session output. Pipeline/coordinator logic never talks to a UI directly, and no state exists only in the UI. The web settings write path is separate: disk remains authoritative, and only a successful atomic save is applied to the coordinator.
...
- **description** — a ticket's free-form body, from its H1 down to the first section JFDI owns. With the title, the open questions and the decision blocks folded into phase comments, it is the slice a stage prompt reads; nothing else in the note is.
...
- **round** — one feedback cycle ending at another agent: Implementation → gate → reviews, with an in-stage gate-fix cycle after either Implementation or QA makes the gate red. Gate failures alone do not consume a round; a post-QA fix that widens QA's initial path scope does.
- **rejection** — a review stage's fail verdict; the unit counted against that reviewer's budget.
...
- **harness** — the agent-session abstraction (`spawn(prompt, options) → event stream`, plus interactive launch); Claude Code and Codex are implementations.
...
- **continuation** — re-entering a stage's own previous agent session later in the same run (`claude -p --resume` / `codex exec resume`) with a short brief, either in a later round or for an in-stage gate fix, instead of starting fresh. Each stage's first session in a run is fresh; a forgotten session falls back to one fresh spawn.
...
- Assert what the operation needs; sanitize only what the sink can't survive. At a boundary (`board.md`, ticket notes, anything `JSON.parse`d, harness stream events, subprocess output) assert what the downstream step can't proceed without — presence, a value in the required range, cross-call invariants, exhaustiveness (`never` checks). Scrub a value only when the destination rejects it (a NUL breaks `git commit -m`) or is harmed by it (a terminal escape sequence poisons every later `git log`); a format you only *asked* an agent for (a 72-char subject) is a steer, not a law — when output misses it the default is to pass it through as-is, neither rejecting it nor truncating/padding it into shape; reshape only when the sink structurally needs it (a commit's first line *is* its subject), and let the sink be the reason, never the number you asked for. Don't scrub a value JFDI itself just produced, and don't normalize input no ticket told you to touch — that's over-engineering (see Simplicity first). Impossible states get an assertion, not a recovery path. Asserting what types already guarantee is noise.
- JFDI's inputs are largely LLM output, so be judicious about which category a requirement falls into. Much of what we hand an agent is a target for the *spirit* of the result — a 72-char subject, a tone, a shape — meant to steer, not a law its output must satisfy; enforcing those literally is the over-defense above. Some requirements genuinely are strict — a JSON shape the next step parses, an enum the code switches on — and that data must conform. Decide which before you defend: enforce the strict ones; let the spirit ones through as-is when they miss — coercing them into shape (truncating to the length we named, padding, reformatting) is just the over-defense in another costume. "We asked for 72 characters" is usually a reason to pass longer text through, not to cut it.
...
- Decide, log, proceed: state assumptions and interpretation choices in your verdict's `decisions` (the pipeline folds them into the producing stage's phase comment) *before* implementing. Never pick between plausible readings silently; escalate only when blocked, with a recommended answer.
- Simplicity first: minimum code that solves the ticket. No speculative features, abstractions for single-use code, unrequested configurability, or handling for impossible states (those get assertions). Speculation wears defensive costumes too — an unrequested normalization, or a scrub with no failure to point to, is scope creep like any feature. Review question: what here is not required by the ticket — including any line of defense with no named, reachable failure?
...
No PO/orchestrator agent, no PRD building or auto-decomposition, no pre-implementation ticket review, no standalone question queue, no multi-project support. Ticket dependency handling is limited to **`blocked-by` gating at dispatch and deadlock detection at any loop size**: a card whose ticket lists a `blocked-by` ticket that is not yet in the done column is not dispatched, and every member of a loop touching the begin column moves to Blocked with the loop named in its ticket comment. The tool reports the loop and never auto-resolves it. Still out of scope — transitive scheduling, critical paths, and auto-ordering. Don't build toward these; just don't preclude the extension seams (ticket sources, merge targets, harnesses, renderers — see [docs/architecture/overview.md](docs/architecture/overview.md)).

## Build sequence

All three milestones are implemented and tested:

1. **Single-ticket pipeline** — `jfdi run` ([pipeline.ts](src/pipeline.ts), [integrate.ts](src/integrate.ts))
2. **Coordinator** — `jfdi start` ([coordinator.ts](src/coordinator.ts), TUI in [src/tui/App.tsx](src/tui/App.tsx), web front end in [src/web/server.ts](src/web/server.ts)), plus `status`/`logs`/`merge`
